...
public class on_click_btn_vehicle_list extends AppCompatActivity implements Vehicle_Adapter.MenuStateChangeListener {
    final String[] Values_from = new String[]{DataBaseHelper._ID,
            DataBaseHelper.VEHICLE_NAME};

    final int[] Values_to = new int[]{R.id.vehicle_id, R.id.vehicle_name};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = dbManager.vehicle_fetch();
        ListView listView = findViewById(R.id.list_view);
        TextView NoOrders = findViewById(R.id.empty);
        NoOrders.setText(R.string.empty_vehicle);
        NoOrders.setGravity(Gravity.CENTER);
        listView.setEmptyView(NoOrders);

        adapter = new Vehicle_Adapter(this, R.layout.on_click_btn_vehicle, cursor, Values_from, Values_to, 0);
...
//                TextView vehicle_idTextView = view.findViewById(R.id.vehicle_id);
//                TextView vehicle_nameTextView = view.findViewById(R.id.vehicle_name);
//
//                String vehicle_id = vehicle_idTextView.getText().toString();
//                String vehicle_name = vehicle_nameTextView.getText().toString();
//
//                Intent modify_intent = new Intent(getApplicationContext(), modify_vehicle.class);
//                modify_intent.putExtra("id", vehicle_id);
//                modify_intent.putExtra("name", vehicle_name);
...
//        FloatingActionButton fab = findViewById(R.id.fab);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.cancel_menu) {
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(on_click_btn_vehicle_list.this);
            builder.setMessage("Are you sure you want to delete?");
            builder.setPositiveButton("Yes", (dialog, which) -> {
                // Handle the action item click here
                boolean is_deleted = false;
                for (String str : selectedFiles) {
                    dbManager.delete_vehicle(Long.parseLong(str));
                    is_deleted = true;
                }
                if (!is_deleted) {
